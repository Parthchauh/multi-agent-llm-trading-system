...
def test_single_agent_config_is_normalized_and_runnable_with_an_executor() -> None:
    config = _config()
    assert config.symbols == ("TEST",)
    assert config.is_runnable_in_current_sprint
    assert "Sprint 8" in config.implementation_note
